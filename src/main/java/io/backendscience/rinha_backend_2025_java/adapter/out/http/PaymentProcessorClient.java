...
public class PaymentProcessorClient implements PaymentProcessorGateway {
    ...
    public void sendPaymentToDefault(PaymentDetail paymentDetail, OffsetDateTime requestedAt) {
        ...
        ResponseEntity<Void> response = restClient
                ...
                .onStatus(HttpStatusCode::is5xxServerError, (req, res) -> {
                    logger.severe(String.format("Error %s to send DEFAULT payment.",  res.getStatusCode()));
                })
                .toBodilessEntity();
    ...
    public void sendPaymentToFallback(PaymentDetail paymentDetail, OffsetDateTime requestedAt) {
        ...
        ResponseEntity<Void> response = restClient
                ...
                .retrieve()
                .onStatus(HttpStatusCode::is4xxClientError, (req, res) -> {
                    logger.warning(String.format("Error %s to send FALLBACK payment.",  res.getStatusCode()));
                })
                .onStatus(HttpStatusCode::is5xxServerError, (req, res) -> {
                    logger.severe(String.format("Error %s to send FALLBACK payment.",  res.getStatusCode()));
                })
                .toBodilessEntity();
